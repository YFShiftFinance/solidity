...
)
{
	...
	if (_baseType.isNegative())
	{
		...
		// That is, the power `baseValue**exponentUpperbound` is positive
		if (exponentUpperbound % 2 == 0)
			solAssert(
				boost::multiprecision::pow(bigint(signedValue), exponentUpperbound) <= _commonType.maxValue() &&
				// underflow
				boost::multiprecision::pow(bigint(signedValue), exponentUpperbound + 1) < _commonType.minValue(),
				"Incorrect exponent upper bound calculated."
			);
		else
			solAssert(
				boost::multiprecision::pow(bigint(signedValue), exponentUpperbound) >= _commonType.minValue() &&
